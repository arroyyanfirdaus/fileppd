...
function insertData($db, $query, $params)
{
...
foreach ($uploadedFiles as $key => &$value) {
    if (!empty($_FILES[$key]['name'])) {
        $value = uploadFile($_FILES[$key], $uploadDir);
    }
}
...
try {
    ...
    $querySiswa = "INSERT INTO data_siswa (
        nisn, nama_siswa, nik, jenis_kelamin, tempat_lahir, tgl_lahir, agama,
        golongan_darah, alamat, email_siswa, rumah_milik, no_telp_rumah, no_telp_siswa,
        asal_sekolah, alamat_asal_sekolah, jurusan, prestasi, hobi
    ) VALUES (
        :nisn, :nama_siswa, :nik, :jenis_kelamin, :tempat_lahir, :tgl_lahir, :agama,
        :golongan_darah, :alamat, :email_siswa, :rumah_milik, :no_telp_rumah, :no_telp_siswa,
        :asal_sekolah, :alamat_asal_sekolah, :jurusan, :prestasi, :hobi
    ...
    if (insertData($db, $querySiswa, $paramsSiswa)) {
        $idSiswa = $db->lastInsertId();

        $queryOrangTua = "INSERT INTO data_orang_tua (
            id_siswa, nama_ayah, pekerjaan_ayah, nama_ibu, pekerjaan_ibu, no_telepon_ortu
        ) VALUES (
            :id_siswa, :nama_ayah, :pekerjaan_ayah, :nama_ibu, :pekerjaan_ibu, :no_telepon_ortu
        )";
        ...
        $queryWali = "INSERT INTO data_wali (
            id_siswa, nama_wali, pekerjaan_wali, alamat_wali, no_telepon_wali
        ) VALUES (
            :id_siswa, :nama_wali, :pekerjaan_wali, :alamat_wali, :no_telepon_wali
        )";
        ...
        $queryBerkas = "INSERT INTO data_berkas (
            id_siswa, foto_kk_ktp_akta_kelahiran, foto_pkh_kis_kks, foto_rapor
        ) VALUES (
            :id_siswa, :foto_kk_ktp_akta_kelahiran, :foto_pkh_kis_kks, :foto_rapor
        )";
        ...
        $queryPendaftaran = "INSERT INTO status_pendaftaran (
            id_siswa, status, catatan_panitia
        ) VALUES (
            :id_siswa, :status, :catatan_panitia
        )";
        ...
        if (
            insertData($db, $queryOrangTua, $paramsOrangTua) &&
            insertData($db, $queryWali, $paramsWali) &&
            insertData($db, $queryBerkas, $paramsBerkas) &&
            insertData($db, $queryPendaftaran, $paramsPendaftaran)
        ) {
            $db->commit();
